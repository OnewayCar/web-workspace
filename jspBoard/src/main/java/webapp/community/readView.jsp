...
<body>
	<main id="read">
		...
		<div style="width: 80%; margin: auto; max-width: 760px;">
			<ul id="table">
				<!-- c:out value 에 들어갈 프로퍼티는 Community 클래스를 보세요. -->
				<li>
					<ul class="row">
						<li>제목</li>
		...
		function formexecute(f){
			...
			document.forms[0].action=url	// form태그의 action속성으로 들어갈 url
			...
			<form action="commentAction.jsp" method="post">
				...
				<!-- 댓글 추가할 메인글의 idx(댓글테이블 mref.고정값)  -->
				<input type="hidden" name="idx" value="0">
				<!-- 삭제할 댓글의 idx(고정값 아님)는 executeCmt 함수에서 설정  -->
				<input type="hidden" name="f" value="0">
				<!-- value가 1이면 등록, 2이면 삭제 -->
				...
				<ul>
					<li>
						<ul class="row">
							<li>작성자</li>
							<li><input name="writer" class="input" value="${user.id }"
								readonly></li>
						</ul>
					</li>
					<li>
	...
	<script type="text/javascript">
function executeCmt(fval,cidx){	/* 첫번쨰는 등록 또는 삭제 기능. 두 번째는 삭제할 댓글 */
	const frm = document.forms[1]
	if(fval===1){	/* 댓글등록. */
		if(frm.content.value==''){
			alert('글 내용은 필수 입력 입니다.');
			frm.content.focus()
			return
		}else{
			frm.f.value= fval
